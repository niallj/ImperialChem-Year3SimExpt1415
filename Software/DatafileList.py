...
class DatafileList(Tk.Listbox):
    ...
    def load(self):
        ...
        if name in self.name2ds.keys():
            ...
            if self.name2ds[name].filetype == "Unknown file type":
                import tkMessageBox
                tkMessageBox.showerror("Unknown file type", "That sort of output file is not recognised.")
                del self.name2ds[name]
            else:
